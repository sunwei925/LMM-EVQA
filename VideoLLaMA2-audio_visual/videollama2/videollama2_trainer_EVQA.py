...
class VideoLLaMA2Trainer(Trainer):
    def __init__(self, model = None, args = None, data_collator = None, train_dataset = None, eval_dataset = None, tokenizer = None, 
                model_init = None, compute_metrics = None, callbacks = None, optimizers = (None, None), preprocess_logits_for_metrics = None):
        super(VideoLLaMA2Trainer, self).__init__(model, args, data_collator, train_dataset, eval_dataset, tokenizer, 
    ...
    def prediction_step(
        ...
        """
        has_labels = False if len(self.label_names) == 0 else all(inputs.get(k) is not None for k in self.label_names) #['labels mos]
        ...
        with torch.no_grad():
            if has_labels or loss_without_labels: #true or false
        ...
        ## skip unused
        # labels_logits = inputs['labels']
        # logits = logits[:, -labels_logits.shape[-1]:]
        ...
        #     # toks_index = -5
        #     # output_logits.append(logits[i,toks_index,:])
        #     toks_index = 0
        #     for toks_index in range(labels_logits.shape[-1]):
        #         if labels_logits[i,toks_index] in self.ids_:
        #             all_toks_index.append(toks_index-1)
        #             output_logits.append(logits[i,toks_index-1,:])
        #             toks_index = toks_index+1
        #             break

        # output_logits = torch.stack(output_logits)
        # # print(preds[:, toks_index])
        

        # logits = output_logits[:,self.ids_]
