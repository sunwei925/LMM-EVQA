...
def frame_sample(duration, mode='uniform', num_frames=None, fps=None):
    if mode == 'uniform':
        ...
        assert fps is not None and num_frames is not None, "Both FPS and number of frames must be provided for first_n_seconds sampling."
        total_frames = duration  # total frames available
        frames_per_second = fps
        
        if total_frames < num_frames * frames_per_second:
            # If video is shorter than num_frames seconds, do uniform sampling
            return np.linspace(0, total_frames-1, num_frames, dtype=int)
        else:
            # Take one frame per second for the first num_frames seconds
            return np.array([int(i * frames_per_second) for i in range(num_frames)])
    else:
...
def process_video(video_path, processor, s=None, e=None, aspect_ratio='pad', num_frames=NUM_FRAMES, va=False, sample_mode='uniform'):
    if isinstance(video_path, str):
        ...
        if num_frames is None:
            ...
            sampled_frame_indices = [frame_indices[i] for i in frame_sample(duration, mode=sample_mode, num_frames=num_frames, fps=fps)]
    ...
    if va:
        ...
        if sample_mode == 'first_n_seconds':
            # For first_n_seconds mode, we only want audio from the first num_frames seconds
            # If video is shorter than num_frames seconds, use the entire video
            clip_duration = min(num_frames, video_duration_seconds)
        else:
            clip_duration = video_duration_seconds
        audio = process_audio_from_video(video_path, clip_duration)
